...
void tratarSennal1()//Definimos el coportamiento de la función tratarsennal1
{
  for (int i = 0; i < 100; i++)
  {
    printf("%d\n", i);
...
void tratarSennal2()//Definimos el coportamiento de la función tratarsennal2
{
  for (int i = 0; i < 100; i++)
  {
    printf("%d\n",i*10);
...
int main()
{
  ...
  switch(pid) //En base al valor de pid cada proceso ejecutará una función
  {
    ...
    default:
      ...
      sleep(3); //Para que le de tiempo al hijo a ejecutar los signal()
      kill(pid, SIGUSR1);//El padre le manda la primera señal al hijo
      kill(pid, SIGUSR2);//El padre le manda la segunda señal al hijo
      sleep(3); //Para que le de tiempo al hijo a ejecutar las señales
